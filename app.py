...
from datasets.crop_knowledge import CROP_KNOWLEDGE
from datasets.disease_knowledge import DISEASE_INFO
from datasets.crop_advisory import CROP_ADVISORY
from datasets.yield_tips import YIELD_TIPS
...
def generate_pdf(report):
    ...
    # Basic Info
    content.append(Paragraph(f"<b>Crop:</b> {report['crop']}", styles['Normal']))
    content.append(Paragraph(f"<b>Expected Yield:</b> {report['yield']}", styles['Normal']))
    content.append(Spacer(1, 10))

    # Environment Info
    content.append(Paragraph("<b>Soil:</b> " + report['soil'], styles['Normal']))
    content.append(Paragraph("<b>Climate:</b> " + report['climate'], styles['Normal']))
    content.append(Paragraph("<b>Season:</b> " + report['season'], styles['Normal']))
    content.append(Paragraph("<b>Sowing Time:</b> " + report['sowing'], styles['Normal']))
    content.append(Paragraph("<b>Harvest Time:</b> " + report['harvest'], styles['Normal']))
    content.append(Spacer(1, 10))

    # Irrigation
    content.append(Paragraph("Irrigation Advice", styles['Heading2']))
    content.append(Paragraph(report['irrigation_main'], styles['Normal']))
    content.append(Spacer(1, 10))

    content.append(Paragraph("Irrigation Tips", styles['Heading2']))
    for tip in report['irrigation_tips']:
        content.append(Paragraph(f"- {tip}", styles['Normal']))
    content.append(Spacer(1, 10))

    # Fertilizer
    content.append(Paragraph("Fertilizer Recommendations", styles['Heading2']))
    for f in report['fertilizer']:
        content.append(Paragraph(f"- {f}", styles['Normal']))
    content.append(Spacer(1, 10))

    # Diseases (FULL DETAILS)
    content.append(Paragraph("Diseases", styles['Heading2']))
    for d in report['diseases']:
        content.append(Paragraph(f"<b>{d['name']}</b>", styles['Normal']))
        content.append(Paragraph(f"Symptoms: {d.get('symptoms', 'N/A')}", styles['Normal']))
        content.append(Paragraph(f"Cause: {d.get('cause', 'N/A')}", styles['Normal']))
        content.append(Paragraph(f"Prevention: {d.get('prevention', 'N/A')}", styles['Normal']))
        content.append(Paragraph(f"Cure: {d.get('cure', 'N/A')}", styles['Normal']))
        content.append(Spacer(1, 8))

    # Best Practices
    content.append(Paragraph("Best Practices", styles['Heading2']))
    for p in report['best_practices']:
        content.append(Paragraph(f"- {p}", styles['Normal']))
    content.append(Spacer(1, 10))

    # Yield Tips
    content.append(Paragraph("Yield Optimization Tips", styles['Heading2']))
    for y in report['yield_tips']:
        content.append(Paragraph(f"- {y}", styles['Normal']))
...
def crop_page():
    ...
    if request.method == "POST":
        try:
            ...
            result = recommend_crop(features).strip().lower()
            ...
            advisory = CROP_ADVISORY.get(result)

            print("Predicted:", result)
            print("Advisory keys:", CROP_ADVISORY.keys())

        except Exception as e:
            flash("Error: " + str(e), "danger")
    return render_template("crop.html", prediction=prediction, advisory=advisory)
...
def yield_page():
    try:
        if request.method == "POST":
            ...
            tips = YIELD_TIPS.get(crop_clean)

            return render_template("yield.html", predicted=round(predicted, 2), tips=tips)

        # If GET request → just render page
        return render_template("yield.html", predicted=None, tips=None)
...
def disease_page():
    ...
    if request.method == "POST":
        try:
            ...
            disease = disease_labels[class_idx]
            confidence = round(float(np.max(preds)) * 100, 2)

            # 🔥 GET DISEASE DETAILS (ADD HERE)
            ...
            prediction = disease.replace("_", " ").replace("___", " - ")
...
def full_report():
    ...
    if request.method == "POST":
        try:
            ...
            mapped_crop = CROP_MAP.get(crop.lower(), crop)
            from models_utils import load_yield_model
            encoders = load_yield_model()["encoders"]
            if mapped_crop not in encoders["Item"].classes_:
              mapped_crop = "Maize"
            ...
            yield_pred = predict_yield({
                ...
                "avg_temp": temp,
                "pesticides_tonnes": rainfall * 0.2,
                "Area": "India",        # 👈 REQUIRED
                "Unnamed: 0": 0         # 👈 DUMMY (safe)
            })

            irrigation_text = irrigation_advice([50, temp, humidity, rainfall])

            # Normalize crop name
            crop_clean = normalize_crop_name(crop)

            # Get knowledge
            crop_info = CROP_KNOWLEDGE.get(crop_clean, {})

            # FINAL REPORT
            report = {
                "crop": crop,
                "yield": round(float(yield_pred), 2),
                "irrigation_main": irrigation_text,

                "soil": crop_info.get("soil", "N/A"),
                "climate": crop_info.get("climate", "N/A"),
                "season": crop_info.get("season", "N/A"),
                "sowing": crop_info.get("sowing_time", "N/A"),
                "harvest": crop_info.get("harvest_time", "N/A"),

                "diseases": crop_info.get("diseases", []),
                "irrigation_tips": crop_info.get("irrigation", {}).get("general", []),
                "fertilizer": crop_info.get("fertilizer", []),
                "best_practices": crop_info.get("best_practices", []),
                "yield_tips": crop_info.get("yield_tips", [])
            }
...
def knowledge_page():
    ...
    if request.method == "POST":
        ...
        crop_data = CROP_KNOWLEDGE.get(selected_crop.lower())
...
def download_report():
    ...
    return send_file(pdf, as_attachment=True, download_name="SmartAgri_Report.pdf")
